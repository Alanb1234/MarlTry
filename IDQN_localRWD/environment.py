...
class MultiAgentGridEnv:
    def __init__(self, grid_file, coverage_radius, max_steps_per_episode, num_agents, initial_positions, reward_type='local'):
    ...
    def reset(self):
        ...
        self.previous_coverage_grid = np.zeros_like(self.grid)
        self.current_step = 0
    ...
    def step(self, actions):
        ...
        self.previous_coverage_grid = self.coverage_grid.copy()
        ...
        if self.reward_type == 'global':
            ...
            rewards = [reward / self.num_agents] * self.num_agents
        else:  # 'local'
            rewards = [self.calculate_local_reward(i) for i in range(self.num_agents)]
    ...
    def calculate_local_reward(self, agent_index):
        local_coverage = np.zeros_like(self.grid)
        self.cover_area_on_grid(self.agent_positions[agent_index], local_coverage)
        
        # New area covered by this agent
        new_area = np.sum((local_coverage > 0) & (self.previous_coverage_grid == 0))
        
        # Overlap with other agents' coverage
        overlap = np.sum((local_coverage > 0) & (self.coverage_grid > local_coverage))
        
        # Encourage exploration of uncovered areas
        exploration_bonus = np.sum((local_coverage > 0) & (self.coverage_grid == local_coverage))
        
        reward = new_area + 0.5 * exploration_bonus - overlap
        return reward
