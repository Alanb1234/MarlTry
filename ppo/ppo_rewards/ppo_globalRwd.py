...
def visualize_multi_agent_model(model, env, steps=50):
    ...
    for step in range(steps):
        ...
        state, reward, done, truncated, info = unwrapped_env.step(actions)
        unwrapped_env.render(ax=ax, actions=actions, episode=episode, reward=info["reward"])
        if done:
            episode += 1
            state, _ = unwrapped_env.reset()
        plt.draw()
...
class RenderCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        return True
#60000
def train_multi_agent_model(env, eval_env, total_timesteps=10):
