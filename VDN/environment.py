...
class MultiAgentGridEnv:
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
            ...
            if self.is_valid_move(new_pos, sensor_readings[i], action):
                new_positions.append(new_pos)
                actual_actions.append(action)
            else:
                new_positions.append(self.agent_positions[i])
                actual_actions.append(4)  # 4 represents 'stay'

        self.agent_positions = new_positions
        self.update_coverage()
        reward = self.calculate_global_reward()
        done = self.current_step >= self.max_steps_per_episode
        return self.get_observations(), reward, done, actual_actions
    ...
    def is_valid_move(self, new_pos, sensor_reading, action):
        ...
        if self.grid[y, x] == 1:  # Check for obstacles
            return False
        if new_pos in self.agent_positions:  # Check for other agents
            return False
        # Check sensor readings
        if action == 0 and sensor_reading[0] == 1:  # up
            return False
        elif action == 1 and sensor_reading[1] == 1:  # down
            return False
    ...
    def get_sensor_readings(self):
        ...
        for pos in self.agent_positions:
            x, y = pos
            reading = [
                1 if y == self.grid_size - 1 or self.grid[y + 1, x] == 1 or (x, y + 1) in self.agent_positions else 0,  # up
                1 if y == 0 or self.grid[y - 1, x] == 1 or (x, y - 1) in self.agent_positions else 0,  # down
                1 if x == 0 or self.grid[y, x - 1] == 1 or (x - 1, y) in self.agent_positions else 0,  # left
                1 if x == self.grid_size - 1 or self.grid[y, x + 1] == 1 or (x + 1, y) in self.agent_positions else 0  # right
            ]
